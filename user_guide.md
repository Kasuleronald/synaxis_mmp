...
### Events
A month calendar — big day tiles, chevrons to move a month at a time, a "Today" button, and a "go to date" jump. Click a day to see everything happening on it; click an event in that list to expand it in place. Create an event with a title, location, start date/time, and optionally an end date/time, and it automatically gets its own public attendance link — the same check-in page people scan a QR code for or an usher opens on a shared device. Tick "Repeats" to generate a whole series at once (daily, weekly, monthly, or monthly-by-weekday, until a date you choose) — each occurrence is its own full event with its own attendance link and its own debrief, not a single recurring placeholder. "Monthly (by weekday)" covers a schedule a plain date-of-month can't: pick a weekday and any combination of 1st/2nd/3rd/4th/Last — tick 1st through 4th for "every Friday except the month's last one," or just "Last" for "the month's final Friday" (e.g. worship evenings on the first three Fridays, a separate overnight series on just the last one). Optionally tag any recurrence type with a meeting category (e.g. Sunday Service) so a recurring calendar-driven meeting feeds the same repeat-absenteeism tracking as one started from Attendance directly. Once an event's start time has passed, file a debrief (venue, actual attendance, strengths, challenges, recommendations).
...
### Attendance
Start a session by picking one of your church's meeting categories (Sunday Service, Cell Fellowship, ...) and a date/time — or "Other (spot meeting)" for a one-off with no ongoing category. Starting the exact same session twice (same name, same date/time) is rejected as a likely accidental duplicate. Then check people in — search an existing member, or take a walk-in's name and phone (tick "Student" if they are one; a known member's student status already comes from their own profile instead). Duplicate check-ins for the same session are prevented automatically. Delete a mistaken check-in with the trash icon, or promote a walk-in straight into a full Member record (every field Add Member offers, not just name and phone) with "Add as a new member" — their student answer carries over automatically. If a walk-in was instead added as a Member separately after the service, "Link to an existing member" points their check-in at that real record instead of leaving a duplicate lying around. A walk-in who checks in three times under the same phone number without ever being registered gets turned into a Member automatically, with their past visits linked to the new record and a follow-up filed to reach out about registering them properly. A whole session (and every check-in on it) can be deleted from the trash icon on its row on the Attendance page — for a session started by mistake.

The check-in list shows a live "X students · Y non-students" count for the session, and the attendance Excel/PDF export (Reports → Attendance lists, or from the session itself) includes a Student column and a leading # row-number column — so you can extract who attended a program broken down by student vs non-student, not just view it live. The PDF's own date line shows the event's date, not the day you happened to download it.

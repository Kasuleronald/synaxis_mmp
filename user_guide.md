...
The check-in list shows a live "X students · Y non-students" count for the session, and the attendance Excel/PDF export (Reports → Attendance lists, or from the session itself) includes a Student column — so you can extract who attended a program broken down by student vs non-student, not just view it live.

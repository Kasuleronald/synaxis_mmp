...
### Organization Admin
**Branches** — your church's campuses; one is always marked "main," reassignable at any time, each with its own optional leader. Deleting one (files a deletion request like everything else) isn't allowed while it's still the main branch. **Staff & roles** — invite a login, assign its role and branch, and toggle any of the additive permissions (deletion approver, registration approver, cell leadership, Pastor, Fellowships department head, devotional editor). Edit a staff member's name, role, or branch at any time from the Edit icon on their row — handy for fixing a wrong branch assignment after the fact. **Audit log** — a searchable record of logins, deletions, approvals, and most create/update actions across your organization, most recent first.

...
### Events
A month calendar — big day tiles, chevrons to move a month at a time, a "Today" button, and a "go to date" jump. Click a day to see everything happening on it; click an event in that list to expand it in place. Create an event with a title, location, start date/time, and optionally an end date/time, and it automatically gets its own public attendance link — the same check-in page people scan a QR code for or an usher opens on a shared device. Tick "Repeats" to generate a whole series at once (daily, weekly, or monthly, until a date you choose) — each occurrence is its own full event with its own attendance link and its own debrief, not a single recurring placeholder. Once an event's start time has passed, file a debrief (venue, actual attendance, strengths, challenges, recommendations).

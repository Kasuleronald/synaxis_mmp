...
If you forget your password, ask your Org Admin (or, if you are the Org Admin, ask the Platform Administrator) to generate a reset link and send it to you directly — there's no automatic "forgot password" email yet.
...
### Events
Create an event (title, date/time, location) and it automatically gets its own public attendance link — the same check-in page people scan a QR code for or an usher opens on a shared device. Once an event's start time has passed, file a debrief (venue, actual attendance, strengths, challenges, recommendations).
...
### Organization Admin
**Branches** — your church's campuses; one is always marked "main," reassignable at any time, each with its own optional leader. **Staff & roles** — invite a login, assign its role and branch, and toggle any of the additive permissions (deletion approver, registration approver, cell leadership, Pastor, Fellowships department head, devotional editor).

### Settings
Your church's profile (name, logo, country, contact info, currency), theme (six color options — Onyx, the neutral grey, is the default), and terminology — rename "Members," "Households," "Fellowships," "Departments," or "Daily Devotional" to whatever your church actually calls them; the new word appears everywhere that noun shows up.
